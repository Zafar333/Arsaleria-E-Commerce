...
const ProductDetailPageImgAndImgSlider = () => {
    const [signleProductImgs, setSingleProductImgs] = useState(["/homepageImgs/2.webp","/homepageImgs/3.webp","/homepageImgs/4.webp","/homepageImgs/5.webp","/homepageImgs/6.webp","/homepageImgs/7.webp","/homepageImgs/8.webp","/homepageImgs/2.webp","/homepageImgs/3.webp","/homepageImgs/4.webp","/homepageImgs/5.webp"])
  ...
  return (
    ...
    <Image.PreviewGroup   items={signleProductImgs}   >
        <Image  className='max-h-[410px] object-contain' src={signleProductImgs[selectedImgIndx]} />
    </Image.PreviewGroup>
    </div>

        <div className='h-[130px]  flex gap-[20px] items-center px-[5px]  overflow-auto border border-red-400'>
            {signleProductImgs.map((img,ind)=>(
                <img key={ind} onClick={()=>ImgIndxFun(ind)} className={`${selectedImgIndx==ind?"border border-darkGreen":""} bg-whiteGray  rounded-md cursor-pointer max-h-[80px]`} src={img}/>
            ))}
        </div>
</div>
