...
import { setAddToCartModalDispatch, setCartDetailDispatch, setDispatchGrandTotal } from '@/store/cartDetailSlice'
  ...
  // close modal fun is end here
    ...
    const deleteShoppingCartFun=(indx)=>{
      ...
      const minusOrderQuantityFun=(itemData,ind)=>{
        ...
        const temp=cartData?.map((data,i)=> i==ind?{
...
// AddToCart Modal is start from here 
        <Drawer onClose={onClose} title='Shopping Cart' className=''  extra={<Space><IoCloseSharp onClick={onClose}  className=' text-darkGreen text-[25px] cursor-pointer '/></Space>} closeIcon={false} open={AddToCartModalState}>
       
           <div className='flex flex-col gap-[30px] '>
             {cartData?.length>0?(
               cartData?.map((data,ind)=>(
                 <div className='py-[10px] px-[10px] border-2 border-lightGreen ' key={ind}>
                   <div className='flex gap-[5px]'>
                   <img src={data?.img} className='border-2 border-gray-300 max-w-[100px]'/>
                   <div className='flex flex-col gap-[5px]'>
                     <p className='font-Poppins text-[16px]'>{data?.title}</p>
                     <div className='flex gap-[10px]'>
                     <p className='font-Poppins text-[18px]'>Price</p>
                     <p className='font-Poppins text-[18px]'>{data?.productPrice}</p>
                     </div>
                     <div className='flex gap-[25px] items-center mt-[10px]'>
                      <div className='flex max-w-[80px]'>
                       <Button className='w-[10px] p-2 rounded-none text-[18px] font-bold' onClick={()=>minusOrderQuantityFun(data,ind)}>-</Button>
                       <p className='rounded-none w-[60px] bg-white border border-gray-300 text-[16px] flex items-center justify-center'>{data?.quantity}</p>
                      <Button  className='w-[10px] p-2 rounded-none text-[18px] font-bold ' onClick={()=>plusOrderQuantityFun(data,ind)}>+</Button>
                      </div>
             ...
             ):(<p className='font-Poppins text-[18px] text-red-600 text-center'>Cart is empty !</p>)}
       
             <div className='w-full flex flex-col gap-[10px] bg-lightGreen px-[10px] py-[30px] absolute left-0 bottom-0 '>
               <div className='flex justify-between'>
                <p className='font-Poppins text-[20px] text-darkGreen font-bold'>SUBTOTAL:</p>
                <p className='font-Poppins text-[20px] text-darkGreen font-bold'>{tot}</p>
               </div>
               <Button className='py-[20px] text-[18px] font-Poppins bg-darkGreen w-full text-lightGreen' onClick={gotoCartPageFun}>CHECKOUT</Button>
             </div>
