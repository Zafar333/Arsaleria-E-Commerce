...
const UserForgotPasswordModal = ({ setUserUpdatedPasswordSuccessdModal, setUserForgotOtpModal, setUserForgotPasswordModal, userForgotPasswordModal }) => {
    ...
    const onFinish = (values) => {
        ...
        toast.error("onfinish server error")
    }
    };
    ...
    const updatePasswordFun = async (pass) => {
        try {
            const res = await fetch(`${DevelopmentBaseUrl}${userEndPoints?.updateForgotPassword}`,
                {
                    method: "POST",
                    headers: {
    ...
    return (
        <Modal className='' onCancel={handleCancel} open={userForgotPasswordModal} footer={false} mask={{ closable: false }} keyboard={false}>
            <IoArrowBackOutline className='text-[20px] text-gray-400 cursor-pointer mb-[8px]' onClick={goToForgotOtpModal} />
            <div className='flex flex-col gap-2 xs:gap-3'>
                <p className=' text-[16px] sm:text-[20px] text-darkGreen'>Set a new password</p>
                <p className='text-gray-400 font-Poppins xs:text-[16px] text-[10px]'>Create a new password. Ensure it differs from
            ...
            >
                <Form.Item
                    ...
                    label={<span className=" text-darkGreen text-[13px] mb-[0px] sm:mb-[7px]">Password</span>}
                    name="password"
                    rules={[
                        {
                            required: true,
                ...
                <Form.Item
                    ...
                    label={<span className="text-darkGreen text-[13px] mb-[7px]">Confirm Password</span>}
                ...
                <Button htmlType='submit' className='w-full text-[14px] sm:text-[18px] text-darkGreen font-Poppins bg-lightGreen py-[20px]'>Update Password</Button>
            </Form>
