...
import { ProductionBaseUrl } from "@/utils/api/main";
...
const UserForgotPasswordOtpModal = ({
  ...
  const userVerifyOtpFun = async (dat) => {
    ...
    try {
      ...
      let res = await fetch(
        `${ProductionBaseUrl}${userEndPoints.resetPasswordVerifyOtp}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 xs:gap-3">
        ...
        <p className="text-gray-400 font-Poppins sm:text-[16px] text-[12px]">
          We sent a code on your email enter 5 digit code that mentioned in the
          email
        </p>
      </div>
      <Form
      ...
      >
        <Form.Item
          className="mt-[2px]"
          label={
            <span className="text-darkGreen text-[12px] sm:text-[14px] mb-[7px]">
              Enter 5-digit OTP
            </span>
          }
          name="otp"
          ...
          <div className="mt-[5px] xs:mt-0 flex gap-[10px] sm:gap-[20px] justify-center">
            {otp?.map((digit, index) => (
              <Input
                ...
                className=" w-[33px] h-[33px] xs:h-[40px] xs:w-[40px] text-center text-[18px]"
              />
            ))}
          </div>
        </Form.Item>
        {btnLoader == false ? (
          <Button
            htmlType="submit"
            className="w-full text-[16]! sm:text-[18px]! text-darkGreen! font-Poppins! bg-lightGreen! py-[16px] md:py-[20px]!"
          >
            Verify
          </Button>
        ) : (
      ...
      <div className="text-center mt-[10px]">
        <label className="text-darkGreen text-[10px] sm:text-[14px]">
          Haven’t got the email yet?{" "}
          <a className=" font-Poppins text-blue-600">Resend email</a>
        </label>
      </div>
    </Modal>
  );
