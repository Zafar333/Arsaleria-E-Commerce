...
import { Button, Checkbox, Dropdown,Space, Form, Input, Select, Radio } from 'antd';
import { DownOutlined } from '@ant-design/icons';
import { AiTwotoneDelete } from "react-icons/ai";
import React, { useEffect, useState } from 'react'
import { useRouter } from 'next/navigation';
import { useDispatch, useSelector } from 'react-redux';
import { setCartDetailDispatch, setDispatchGrandTotal } from '@/store/cartDetailSlice.js';
...
const CheckoutPage = () => {
  ...
  const cartItemsData=useSelector((state)=>state.cartDetailSlice.cartdetail)
  const navigate=useRouter()
  const [cartItems,setCartItems]=useState([{img:"/cover1.svg",title:"shirt round neck",quantity:1,productPrice:"1899", deliveryCharges:"200"}
    ,{img:"/cover1.svg",title:"shirt round neck",quantity:1,productPrice:"1899", deliveryCharges:"200"}
    ,{img:"/cover1.svg",title:"shirt round neck",quantity:1,productPrice:"1899", deliveryCharges:"200"}
    ,{img:"/cover1.svg",title:"shirt round neck",quantity:1,productPrice:"1899", deliveryCharges:"200"}
  ])
...
// form onfinishfailed fun is start from  here
  ...
  const minusOrderQuantityFun=(itemData,ind)=>{
    ...
    const temp=cartItemsData?.map((data,i)=> i==ind?{
      ...data,quantity:itemData.quantity-1
    }
    :data
  ...
  // sendpaymentfun is start from here
    const sendPaymentFun=async()=>{
      ...
      const data = await fetch("http://localhost:5000/user/create-payment-intent", {
        method: "POST",
  ...
  return (
   ...
   <div className='flex flex-col'>
      ...
      <div className='mx-[10px] sm-mx-[40px] flex flex-col gap-[10px] bg-darkGreen p-[10px] rounded-sm'>
        <p className='font-Poppins text-[20px] text-lightGreen'>Order Summary</p>
        <div className='flex flex-col gap-[5px] px-[20px]'>
        <div className='flex justify-between'>
        <p className='font-Poppins tex-[14px] sm-text-[16px] text-lightGreen'>Delivery Charges </p>
        <p className='font-Poppins text-[14px] sm-text-[16px] text-lightGreen'> {cartItemsData?.length>0?(cartItemsData[0]?.deliveryCharges):(0)} </p>
        </div>
        <div className='flex justify-between'>
        <p className='font-Poppins text-[14px] sm-text-[16px] text-lightGreen'>GrandTotal  </p>
        <p className='font-Poppins text-[14px] sm-text-[16px] text-lightGreen'>{total} </p>
        </div>
        </div>
      </div>
      {/* order summmary section is end here */}
    {/* <div className='h-[500px] overflow-auto'> */}
      {cartItemsData?.length>0?(cartItemsData?.map((item,ind)=>(
    <div className='m-[10px] bg-lightGreen rounded-sm sm:m-[40px] p-[20px]' key={ind}>
      <div className='flex flex-col gap-[20px] '>
       <div >
        <div className='flex gap-2'>
         <img src={item?.img} className='w-[70px] border border-darkGreen ' />
         <p className='text-darkGreen font-Poppins text-[16px]'>{item?.title}</p>
        </div> 
       <div className='flex gap-[25px] items-center mt-[10px]'>
        <div className='flex max-w-[80px]'>
         <Button className='w-[10px] p-2 rounded-none text-[18px] font-bold' onClick={()=>minusOrderQuantityFun(item,ind)}>-</Button>
         <p className='rounded-none w-[60px] bg-white border border-gray-300 text-[16px] flex items-center justify-center'>{item?.quantity}</p>
         <Button  className='w-[10px] p-2 rounded-none text-[18px] font-bold ' onClick={()=>plusOrderQuantityFun(item,ind)}>+</Button>
        </div>
        <AiTwotoneDelete className='text-[24px] text-red-500 cursor-pointer' onClick={()=>deleteShoppingCartFun(ind)}/>
       </div>
       </div>
       <div className='flex flex-col gap-[8px]' >
        <div className='flex justify-between'>
        <label className='font-Poppins text-[16px]'>Price</label>
        <label className='font-Poppins text-[16px]'>{item?.productPrice}</label>
        </div>
        <div className='flex justify-between'>
        <label className='font-Poppins text-[16px]'>Quantity</label>
        <label className='font-Poppins text-[16px]'>{item?.quantity}</label>
        </div>
        <div className='flex justify-between'>
        <label className='font-Poppins text-[16px]'>Total</label>
        <label className='font-Poppins text-[16px]'>{Number(item?.productPrice*item?.quantity)}</label>
        </div>
      </div>
      </div>

    </div>
      ))):(<div className='font-Poppins text-[20px] text-center text-red-500 mt-[50px]'>your Cart is empty</div>)}
  ...
  {/* checkout Product information  card is end here */}
   ...
   <div className='bg-white py-[30px] px-[10px] sm:p-[30px] rounded-sm h-fit'>
    <p className='font-Poppins text-[22px] mb-[20px] text-darkGreen'>Delivery Details</p>
  ...
  >
   ...
   >
     <Radio.Group >
            <Radio onChange={checkPaymentMethodFun} value="Cash On Delivery">Cash On Delivery </Radio>
            <Radio onChange={checkPaymentMethodFun} value="online card">Debit or Credit Card </Radio>
          </Radio.Group>
     </Form.Item>
      ...
      {payementMethod==true?(  
   <div>
    <p className='font-Poppins text-[22px] mt-[28px] mb-[14px] text-darkGreen'>Card Details</p>
