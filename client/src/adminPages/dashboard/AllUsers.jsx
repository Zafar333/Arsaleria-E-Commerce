...
import { Button, Image, Radio, Spin } from "antd";
     ...
     const [pageLoading, setPageLoading] = useState(true);
      ...
      const columns = [
    ...
    
        {
          ...
          // dataIndex: "action",
    render: (_, record) => (
      <MdDeleteOutline className="text-red-600 text-[17px]" onClick={() => handleDeleteUser(record.id)} />
      ...
      const browserUrlChangePageloadingfun = () => {
        ...
        if (
          `${frontendDevelopmentBaseUrl}/admin/allUsers?id=${queryparam[0]}` ==`${frontendDevelopmentBaseUrl}${pathname}?id=${queryparam[0]}`
        ) {
            // console.log("inblock")
