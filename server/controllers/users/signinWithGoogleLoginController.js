...
const {
  verifyGoogleToken,
} = require("../../middleware/verifySigninWithGoogleToken/verifyGoogleToken");
...
const signinWithGoogleLoginController = async (req, res) => {
  ...
  try {
    ...
    if (!idToken) {
      return res.send({
        status: 400,
        message: "google token invalid credentials",
      });
    }
    ...
    if (user?.rows?.length > 0) {
      ...
      const data = await client.query(
        `UPDATE users SET image=$1, google_id=$2, user_jwt_refereshToken=$3 WHERE id=$4 `,
    ...
    user = await client.query(
      `INSERT INTO users (name, email, image, google_id) VALUES ($1, $2, $3, $4) RETURNING * `,
