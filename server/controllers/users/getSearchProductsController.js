...
WHERE products.product_name ILIKE $1 AND EXISTS (
SELECT 1
    FROM products_variants 
       ...
       AND products_variants.stock_status = $2
        ...
        `,
        [`${req?.query?.productName}%`, "Available", limit],
      );
    ...
    // for cursor infinite scrolling condition start here
    if (limit && cursor) {
      const result = await pool.query(
        `
              SELECT
    ...
    // for cursor infinite scrolling condition end here
  } catch (error) {
    console.log("getSearchProductsController errors", error?.message);
    return res?.send({
      status: 500,
