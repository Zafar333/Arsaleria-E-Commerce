...
// user login req controller is start from here
 const userLogin=async (req,res)=>{
     ...
     if(data?.email && data?.password ){
         let result=await pool?.query(`SELECT id, email, password FROM users WHERE email=$1`,[data?.email])
        //  return console.log("result",result)
         if(result?.rows?.length>0){
             let match=await checkPassword(data?.password,result?.rows[0]?.password)
             if(match){
 
                 //  CREATE REFRESH and ACCESS JWT TOKEN FUN IS START FROM HERE 
                 const accesstoken=await createJwtAccessToken(result?.rows[0],process.env.USER_JWT_ACCESS_TOKEN_SECRET_KEY)
                 const refreshtoken=await createJwtRefreshToken(result?.rows[0],process.env.USER_JWT_REFRESH_TOKEN_SECRET_KEY)
            ...
            if (refreshtoken&& accesstoken) {
                ...
                return res.json({ status: 200, message: "you login successfull", data:result?.rows[0]?.id })
            }
    ...
    console.log("user login page controller error",err?.message)
     res.json({status:500,message:"i am catch server error"})
 }
